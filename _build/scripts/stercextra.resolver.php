...
if ($options['topic']) {
    ...
    $topic     = explode('/', $topic);
    $signature = end($topic);
    $version   = str_replace(str_replace(' ', '', strtolower($package)) . '-', '', $signature);
}

$userNameObj = $modx->getObject(modSystemSetting::class, ['key' => str_replace(' ', '', strtolower($package)) . '.user_name']);
$userName    = ($userNameObj) ? $userNameObj->get('value') : '';
$userEmailObj = $modx->getObject(modSystemSetting::class, ['key' => str_replace(' ', '', strtolower($package)) . '.user_email']);
